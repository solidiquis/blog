# Concurrency in Ruby
### Building a simple multithreaded TCP server
---
...
## What is the GVL?
Ruby was developed well before multicore processors and threading became mainstream; but when they became inevitable, Ruby, a language full of mutable references, needed some way to minimize the likelihood of data races or even memory leaks which might be caused by multiple simultaneously runnings threads accessing shared memory. The simplest approach to reducing the chances of these things happening was to implement some sort of mutex around Ruby's VM, requiring any thread that needs CPU time to acquire the lock before its Ruby code could be interpreted. This is the Global VM Lock.
